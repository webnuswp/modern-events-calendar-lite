...
<form id="mec_book_form<?php echo esc_attr($uniqueid); ?>" class="mec-booking-form-container row" onsubmit="mec_book_form_submit(event, <?php echo esc_attr($uniqueid); ?>);" novalidate="novalidate" enctype="multipart/form-data" method="post">
                    ...
                    <?php /** Radio **/ elseif ($bfixed_field['type'] == 'radio') : ?>
                        <?php foreach ($bfixed_field['options'] as $bfixed_field_option) : ?>
                            <label for="mec_book_bfixed_field_reg<?php echo esc_attr($bfixed_field_id . '_' . strtolower(str_replace(' ', '_', $bfixed_field_option['label']))); ?>">
                                <input type="radio" id="mec_book_bfixed_field_reg<?php echo esc_attr($bfixed_field_id . '_' . strtolower(str_replace(' ', '_', $bfixed_field_option['label']))); ?>" name="book[fields][<?php echo esc_attr($bfixed_field_id); ?>]" value="<?php esc_attr_e(stripslashes($bfixed_field_option['label']), 'modern-events-calendar-lite'); ?>" />
                                <?php esc_html_e(stripslashes($bfixed_field_option['label']), 'modern-events-calendar-lite'); ?>
                            </label>
                        <?php endforeach; ?>
    ...
    <ul class="mec-book-tickets-container">
        ...
        foreach ($tickets as $ticket_id => $count) : if (!$count) continue;
            ...
            for ($i = 1; $i <= $count; $i++) : $j++;
                $ticket_price = $this->book->get_ticket_price($ticket, current_time('Y-m-d'), $event_id, $occurrence);
                ...
                <li class="mec-book-ticket-container <?php echo (($j > 1 and $first_for_all) ? 'mec-util-hidden' : ''); ?>">
                    <?php if (!empty($ticket['name']) || !empty($this->book->get_ticket_price_label($ticket, current_time('Y-m-d'), $event_id, $occurrence))) : ?>
                        <h4 class="col-md-12">
                            ...
                            <?php
                                $ticket_price = $this->book->get_ticket_price_label($ticket, current_time('Y-m-d'), $event_id, $occurrence);
                            ...
                            <?php /** Radio **/ elseif ($reg_field['type'] == 'radio') : ?>
                                <?php $r_value = $this->main->get_from_mapped_field($reg_field); foreach ($reg_field['options'] as $reg_field_option) : ?>
                                    <label for="mec_book_reg_field_reg<?php echo esc_attr($j . '_' . $reg_field_id . '_' . strtolower(str_replace(' ', '_', $reg_field_option['label']))); ?>">
                                        <input type="radio" id="mec_book_reg_field_reg<?php echo esc_attr($j . '_' . $reg_field_id . '_' . strtolower(str_replace(' ', '_', $reg_field_option['label']))); ?>" name="book[tickets][<?php echo esc_attr($j); ?>][reg][<?php echo esc_attr($reg_field_id); ?>]" value="<?php esc_attr_e(stripslashes($reg_field_option['label']), 'modern-events-calendar-lite'); ?>" <?php echo (($r_value and $r_value == esc_attr__(stripslashes($reg_field_option['label']), 'modern-events-calendar-lite')) ? 'checked="checked"' : ''); ?> />
                                        <?php esc_html_e(stripslashes($reg_field_option['label']), 'modern-events-calendar-lite'); ?>
                                    </label>
                                <?php endforeach; ?>
