...
class MEC_skins extends MEC_base
{
    ...
    public function display_organizers($event)
    {
        ...
        if($status and is_object($event) and isset($event->data->organizers) and count($event->data->organizers))
        {
            ...
            {
                $organizer_url = !empty($organizer['url']) ? 'href="'. esc_url($organizer['url']) .'" target="_blank"' : '';
                if(isset($organizer['name']) and trim($organizer['name']))
                {
                    $name_url = trim($organizer_url) ? '<a class="mec-color-hover" '.$organizer_url.'>' . esc_html(trim($organizer['name'])) . '</a>' : '<span>' . esc_html(trim($organizer['name'])) . '</span>';
